...
final class AudioFormatConverter {
    ...
    func convert(sampleBuffer: CMSampleBuffer) -> [Int16]? {
        ...
        return withAudioBufferList(from: sampleBuffer) { bufferList in
            runConverter(bufferList: bufferList, inputASBD: inputASBD, inputFrames: inputFrames)
        }
    }

    private func withAudioBufferList(
        from sampleBuffer: CMSampleBuffer,
        body: (UnsafeMutableAudioBufferListPointer) -> [Int16]?
    ) -> [Int16]? {
        ...
        return body(UnsafeMutableAudioBufferListPointer(bufferListPointer))
    }
